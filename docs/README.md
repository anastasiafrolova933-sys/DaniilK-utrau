...
**Две Google-таблицы** (ресторан «КАЕФ» = F&B Утрау), помесячно:

| Файл | ID | Вкладки |
|------|----|---------| 
| №1 сводка | `10mv7goP8z1z0P8P9AAY2YkAO1UF5atOTSpSMrrIZ-MY` | A1 сводка (gid 1642884690), A2 банкеты (gid 2082985573) |
| №2 iiko | `12IY6jtT4tBI1YHL_Bl6S_VyJoNxA6oWD9pVqdKGTbPg` | B1-B6 продажи по типам, B7-B12 акты списаний |

**Источник правды — A1** (сводный отчёт ДС службы питания): выручка по типам, с/с, списания
(7 категорий), товарный остаток, KPI (гости, средний чек, фудкост по типам). Детальные iiko-вкладки
(B1-B6) дают разбивку по блюдам; сверены с A1 в копейку.

**Парсер — Python** (`scripts/build_fnb.py`, не PowerShell): A1/A2 — рукодельные таблицы с кириллицей
и ₽, PS5.1 их ломает. Парсит по **меткам строк** (а не по индексам — устойчиво к сдвигам). Числа
берутся с колонки C (колонка A = «№ п/п», не значение!). Выход — `data/fnb_data.js` (window.UTRAU_FNB).

**Типы для фильтра** (как просил клиент): А-ля карт (кухня) / Бар (напитки+алкоголь) / Банкеты /
Шведка-завтраки. Маппинг типа → строки A1 зашит в `fnb.html` (функция `typeMetrics`).

**Ежемесячно:** новая выгрузка iiko. Добавить месяц в `MONTHS` в `build_fnb.py` (месяц → gid-вкладки),
запустить `python scripts/build_fnb.py` (скачает и пересоберёт). Вызов уже в `download.ps1` (ежедневный цикл).

**Особенности данных:** вкладки списаний B7-B12 иногда содержат несколько отчётов стопкой
(категоризация в колонке «Счёт списания») — поэтому суммы списаний берём из A1, не из B. Фудкост
(FC) = себестоимость / выручка; норма 25-35%, выше 40% — внимание (в январе завтраки 43,2%).
